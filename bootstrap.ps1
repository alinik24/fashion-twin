param([switch]$SkipInstall,[switch]$TestOnly)
...
if (-not $SkipInstall) { if ($TestOnly -and (Test-Path requirements-test.txt)) { & $py -m pip install -r requirements-test.txt } elseif (Test-Path requirements.txt) { & $py -m pip install -r requirements.txt } }
